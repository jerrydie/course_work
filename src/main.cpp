...
int main(int argc, char** argv) {
	...
	double c1 = 0;
	double c2 = 0;
	for (u8 pt = 0; pt != maximum_pt - 1; pt += 1) // Перебираем все открытые тексты
	{
	...
	c1 = abs(2*c1/maximum_pt - 1);
	c2 = abs(2*c2/maximum_pt - 1);
	...
	std::cout << "alpha|0 experimental correlation: " << c2 << std::endl;
}
