...
int main(int argc, char** argv) {
	...
	u64 mask1 = alpha; //первая маска, вида 0|alpha
	std::cout << "Mask1: " << std::setw(65) << std::bitset<64>(mask1) << std::endl;
	u64 mask2 = (u64)alpha << 4; // вторая маска, вида alpha|0
	std::cout << "Mask2: " << std::setw(65) << std::bitset<64>(mask2) << std::endl;
	//"максимальный" открытый текст, для цикла
	u64 maximum_pt = std::pow(2, pt_bit_length);// - std::pow(2,4);
	//Количество материала
	u64 qtty_pt = std::pow(2,pt_bit_length);
	double c1_mean = 0;
	double c2_mean = 0;
	...
	std::cout << "0|alpha experimental correlation: " << c1_mean/2000 << std::endl;
	std::cout << "alpha|0 experimental correlation: " << c2_mean/2000 << std::endl << std::endl;
